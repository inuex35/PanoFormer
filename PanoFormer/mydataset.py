...
class MyDataset(data.Dataset):
    def __init__(self, root_dir, list_file, height=512, width=1024, disable_color_augmentation=False,
        ...
        if self.color_augmentation:
            try:
                ...
                self.color_jitter = transforms.ColorJitter(brightness=self.brightness, 
    ...
    def __getitem__(self, idx):
        ...
        depth_name = self.rgb_depth_list[idx][1]
        gt_depth = cv2.imread(depth_name, -1)
        gt_depth = cv2.resize(gt_depth, dsize=(1024, 512), interpolation=cv2.INTER_NEAREST)
        gt_depth = gt_depth.astype(np.float16) / 65535.0
        gt_depth[gt_depth > self.max_depth_meters+1] = self.max_depth_meters + 1


        # Save the loaded RGB and depth images
        save_dir = "/notebooks/ue5/before_train"
        rgb_save_path = os.path.join(save_dir, os.path.basename(rgb_name))
        depth_save_path = os.path.join(save_dir, os.path.basename(depth_name))
        cv2.imwrite(rgb_save_path, cv2.cvtColor(rgb, cv2.COLOR_RGB2BGR))
        cv2.imwrite(depth_save_path, (gt_depth.astype(np.float16)))
        ...
        if self.is_training and self.color_augmentation and random.random() > 0.5:
            aug_rgb = np.asarray(self.color_jitter(transforms.ToPILImage()(rgb)))
        else:
            aug_rgb = rgb
        ...
        inputs["filename_rgb"] = rgb_name
        inputs["filename_depth"] = depth_name
